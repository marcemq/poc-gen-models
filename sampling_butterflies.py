...
def sampling(plot_steps, fm_steps, num_samples=16):
    ...
    pbar = tqdm.tqdm(range(1, fm_steps + 1), desc="Sampling")
    for i, t in enumerate(torch.linspace(0, 1, fm_steps, device=device), start=1):
        time_indices = (t * 1000).clamp(0, 999).long()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fm_steps', type=int, default=1000, help='Flow-matching steps to compute')
    args = parser.parse_args()

    sampling(args.plot_steps, args.fm_steps, args.num_samples)